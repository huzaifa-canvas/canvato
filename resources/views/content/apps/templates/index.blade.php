  ...
  <div class="card">
    <div class="card-header border-bottom">
      <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
        <h5 class="mb-0">{{ $currentType ?? 'All Templates' }} <span
            class="badge bg-label-primary ms-2">{{ $templates->total() }}</span></h5>
        ...
        <div class="d-flex flex-column flex-sm-row align-items-sm-center gap-3">
          <form method="GET" action="{{ url()->current() }}" class="d-flex align-items-center gap-2 m-0">
            ...
            <input type="text" name="date_range" id="flatpickr-range" class="form-control form-control-sm"
              style="min-width: 200px;" placeholder="Date Range" value="{{ request('date_range') }}">
            <button type="submit" class="btn btn-primary btn-sm text-nowrap">Filter</button>
          </form>

          @can('create products')
            <button type="button" class="btn btn-label-secondary btn-sm text-nowrap" data-bs-toggle="modal"
              data-bs-target="#bulkUploadModal">
              <i class="icon-base ti tabler-upload me-1"></i> Bulk Upload
            </button>
            <a href="{{ route('templates.create', $currentType ? ['type' => $currentType] : []) }}"
          ...
          @forelse($templates as $template)
            <tr>
              <td>
                ...
                @if ($mainThumb)
                  <img src="{{ asset('storage/' . $mainThumb) }}" alt="Thumbnail" width="50" height="50"
              ...
              <td style="white-space: normal; max-width: 250px; word-wrap: break-word;">
              ...
              <td>
                <div class="d-flex gap-1">
                  @can('edit products')
                    <a href="{{ route('templates.edit', $template->id) }}" class="btn btn-icon btn-text-primary btn-sm"
                      title="Edit">
          ...
          @empty
            <tr>
              <td colspan="{{ $currentType ? 6 : 7 }}" class="text-center py-4">
                ...
                <a href="{{ route('templates.create', $currentType ? ['type' => $currentType] : []) }}">Create your
                  first!</a>
              </td>
            </tr>
          @endforelse
