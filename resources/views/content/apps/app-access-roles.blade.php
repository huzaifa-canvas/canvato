        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
          ...
          <ul class="list-unstyled d-flex align-items-center avatar-group mb-0">
            ...
            @if($role->users_count > 4)
              <li class="avatar">
                <span class="avatar-initial rounded-circle pull-up" data-bs-toggle="tooltip" data-bs-placement="bottom" title="{{ $role->users_count - 4 }} more">+{{ $role->users_count - 4 }}</span>
              </li>
            @endif
          </ul>
        </div>
        <div class="d-flex justify-content-between align-items-end">
          <div class="role-heading">
            ...
            <a href="javascript:;"
               ...
               class="btn btn-icon btn-text-primary btn-sm role-edit-modal"
          ...
          <form action="{{ route('access-roles.destroy', $role->id) }}" method="POST" class="d-inline">
              @csrf
              @method('DELETE')
              <button type="submit" class="btn btn-icon btn-text-danger p-0" onclick="return confirm('Are you sure you want to delete this role?')">
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.querySelectorAll('.role-edit-modal').forEach(function(btn) {
    btn.addEventListener('click', function() {
      ...
      form.action = '/app/access-roles/' + roleId;
