...
@section('page-css')
    <style>
        ...
        .breadcrumb svg { width: 10px; opacity: 0.5; }
        ...
        .thumbnail.active, .thumbnail:hover { opacity: 1; border-color: var(--accent-color); }
        .thumbnail img { width: 100%; height: 100%; object-fit: cover; }

        .product-sidebar { display: flex; flex-direction: column; gap: 30px; }
        .pricing-box {
        ...
        .pricing-box h4 {
            font-size: 16px; margin-bottom: 20px; color: #fff;
        ...
        .license-option.selected { border-color: var(--accent-color); background: rgba(194, 46, 245, 0.05); }
        .license-price { font-family: var(--font-mono); font-size: 18px; font-weight: 700; color: #fff; }

        .btn-cart {
            width: 100%; height: 56px; background: var(--accent-color);
        ...
        .accordion-item { border-top: 1px solid var(--border-light); overflow: hidden; }
        .accordion-item:last-child { border-bottom: 1px solid var(--border-light); }
        .accordion-header {
            padding: 22px 0; font-weight: 700; font-size: 14px; cursor: pointer;
            display: flex; justify-content: space-between; align-items: center;
            text-transform: uppercase; letter-spacing: 0.1em; background: transparent !important;
            border: none; width: 100%; color: var(--text-primary); text-align: left; outline: none;
        }
        .accordion-header::after { content: '+'; font-size: 18px; color: var(--text-secondary); }
        .accordion-item.active .accordion-header::after { content: '-'; color: var(--accent-color); }
        .accordion-content { display: none; padding-bottom: 20px; color: var(--text-secondary); line-height: 1.7; }
        .accordion-item.active .accordion-content { display: block; }

        .tags-list, .features-list { list-style: none; padding: 0; margin: 0; display: flex; flex-wrap: wrap; gap: 8px; }
        .features-list li { width: 100%; display: flex; align-items: center; gap: 10px; margin-bottom: 10px; }
        .features-list li svg { width: 16px; height: 16px; color: var(--accent-color); }
        .tags-list li {
            background: rgba(255,255,255,0.05); padding: 5px 12px; border-radius: 4px;
            font-size: 11px; border: 1px solid var(--border-light); color: var(--text-secondary);
            text-transform: uppercase; letter-spacing: 0.05em;
        }

        .tabs-section { padding: 80px 0; border-top: 1px solid var(--border-light); }
        .tabs-nav { display: flex; gap: 40px; border-bottom: 1px solid var(--border-light); margin-bottom: 50px; }
        .tab-btn {
            background: transparent !important; border: none; color: var(--text-secondary);
            font-size: 16px; font-weight: 700; padding: 0 0 25px 0; cursor: pointer; position: relative;
        }
        .tab-btn.active { color: var(--accent-color) !important; }
        ...
        <section class="product-header-section">
            <div class="container">
                <div class="breadcrumb">
                    ...
                    <a href="{{ route('frontend.templates', ['category' => $template->category->slug]) }}">{{ $template->category->name }}</a>
                    <svg viewBox="0 0 320 512" fill="currentColor"><path d="M278.6 233.4c12.5 12.5 12.5 32.8 0 45.3l-160 160c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3L210.7 256 73.4 118.6c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0l160 160z"/></svg>
                    @endif
                ...
                <p class="product-subtitle">{{ $template->short_description ?? 'A premium creative design asset ready to accelerate your workflow.' }}</p>
            </div>
        </section>
                ...
                <div class="product-grid">
                    ...
                    <div class="product-gallery">
                        <div class="main-image">
                            ...
                            @else
                                <img src="https://images.unsplash.com/photo-1541462608143-67571c6738dd?auto=format&fit=crop&w=1200&h=800&q=80" alt="Main Product Preview">
                            @endif
                        </div>
                    ...
                    <div class="product-sidebar">
                        ...
                        <div class="pricing-box action-buttons">
                            @if($template->price <= 0)
                                @if(auth()->check())
                                    <a href="{{ route('download.secure', $template->slug) }}" class="btn-cart" style="background: #28a745; text-decoration: none;">
                                    ...
                                    @if($template->meta_data)
                                        @foreach($template->meta_data as $key => $value)
                                            @if($key !== 'compatible_tools')
                                                <p><strong>{{ \Illuminate\Support\Str::title(str_replace('_', ' ', $key)) }}:</strong> {{ is_array($value) ? implode(', ', $value) : $value }}</p>
                                            @endif
                                        @endforeach
                                    @endif
                                </div>
                            ...
                            <div class="accordion-item">
                                ...
                                <div class="accordion-content">
                                    <ul class="features-list">
                                        ...
                                        <li><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12"></polyline></svg> {{ $tool }}</li>
                                        @endforeach
                                    </ul>
                                </div>
                            </div>
